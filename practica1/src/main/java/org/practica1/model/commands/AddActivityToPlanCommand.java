...
public class AddActivityToPlanCommand implements ICommand {
    @Override
    public Message execute(String parametros){
        ...
        if (userController.someoneIsLoggedIn()) {
            ...
            if (activityId > 0 && planId > 0) {
                    ...
                    if (activity != null){
                        if(plan.getOwner().equals(userController.getLoggedUser())){
                            if(plan.getActivities().contains(activity)) {
                                message = ErrorMessage.ACTIVITY_ALREADY_IN_PLAN;
                            }else{
                                plan.getActivities().add(activity);
                                message = SuccessMessage.ACTIVITY_ADDED_TO_PLAN;
                            }
                        }else{
                ...
                message = ErrorMessage.CANT_ADD_ACTIVITY_TO_PLAN;
            }
        } else {
